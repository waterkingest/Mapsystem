...
<div id="map">
  <div class="topTool">
            ...
            <div class="info-div">
                <input class="esri-component esri-widget--button esri-widget esri-interactive" type="button" :value="relativeDimension()" @click="toggleDimension()">
            </div>
            <el-switch
              class="clickon"
              v-model="clickon"
              inactive-text="点选获取坐标">
            </el-switch>
        </div>
        <div id="viewDiv">
            <!-- 地图容器 -->
...
export default {
  ...
  methods:{
    ...
    set3dMap: function(){
      var _this=this
      _this.maplayer=new _this.Map({
      basemap:"oceans"
      });
      var mapset={
        map:_this.maplayer,
        container:"map-container",
        center:[109.864091, 34.090375],
        zoom:5
      }
      var map_3d = new _this.SceneView(mapset);
      _this.watchUtils.whenTrue(map_3d, "stationary", function(){

        mapset.center = [map_3d.center.longitude,map_3d.center.latitude];
        if (map_3d.viewpoint) {
          mapset.scale = map_3d.viewpoint.scale;
        }
      });
      var overviewMap = new _this.Map({
      basemap: "topo"
      });
      var mapView = new _this.MapView({
      ...
      mapView.when(function() {
            map_3d.when(function() {
                  setup();
            });
      });

      function setup() {
          const extendsmap = new _this.Graphic({
            geometry: null,
            symbol: {
              type: "simple-fill",
              color: [0, 0, 0, 0.5],
          ...
          mapView.graphics.add(extendsmap);
          _this.watchUtils.init(map_3d, "extent", function(extent) {
            // Sync the overview map location
            // whenever the 3d view is stationary
            mapView
                .goTo({
                  center: map_3d.center,
                  scale:
                    map_3d.scale * 6 *Math.max(

                      map_3d.width / mapView.width,
                      map_3d.height / mapView.height
                    )
                })
    ...
    initMap: function () {
      var _this=this
      this.maplayer= new this.Map({
        basemap: 'topo'
      })
      _this.mainmap=new _this.MapView({
        map:_this.maplayer,
        container:"map-container",
        center:[109.864091, 34.090375],
        zoom:5
      })
      _this.layer1=new _this.MapImageLayer({
        url:"http://cache1.arcgisonline.cn/arcgis/rest/services/ChinaOnlineCommunity/MapServer"
      })
      _this.layer2 = new _this.MapImageLayer({
        url:"http://localhost:6080/arcgis/rest/services/monster/Province/MapServer",
        visible:true,
      });
      _this.layer3 = new _this.FeatureLayer({
        url:"	http://localhost:6080/arcgis/rest/services/monster/monster/FeatureServer",
      ...
      _this.maplayer.add(_this.layer1)
      _this.maplayer.add(_this.layer2)
      _this.maplayer.add(_this.layer3)
      _this.search = new _this.Search({
        view: _this.mainmap,
      });
      _this.mainmap.ui.add(_this.search, "top-right");
      var overviewMap = new _this.Map({
        basemap: "topo"
      });
      var mapView = new _this.MapView({
      ...
      function setup() {
          ...
          mapView.graphics.add(extendsmap);
          _this.watchUtils.init(_this.mainmap, "extent", function(extent) {
            mapView
                .goTo({
                  center: _this.mainmap.center,
                  scale:
                    _this.mainmap.scale * 4 *Math.max(

                      _this.mainmap.width / mapView.width,
                      _this.mainmap.height / mapView.height
                    )
                })
                .catch(function(error) {
                  // ignore goto-interrupted errors
      ...
      _this.mainmap.ui.add("topbar", "top-right");
      this.drawtool();
    },
    locationaddre:function(){
      var _this=this
      var geocoder = _this.search.search(_this.addre);
      var params = {
    ...
    inputlayer1:function(judge){
      var _this=this

      if(judge){
        console.log(judge);
        console.log(_this.maplayer)
        _this.layer2.visible=judge;
			}else if(!judge){
        //console.log(judge);
        console.log(_this.maplayer)
        _this.layer2.visible=judge;
    ...
    inputlayer2:function(judge){
      var _this=this
      if(judge){
				console.log(judge);
				_this.layer3.visible=judge;
			}else if(!judge){
        console.log(judge);
        _this.layer3.visible=judge;
        //_this.maplayer.add(_this.layer1);
    ...
    setlayer:function(value){
      var _this=this
      _this.layer2.opacity=1-value/100;
      console.log(value/100)
    },
    setlayer2:function(value){
      var _this=this
      _this.layer3.opacity=1-value/100;
      console.log(value/100)
    },
    clickaddress:function(){
      var _this=this
      function showPopup(address, pt){
        _this.mainmap.popup.open({
          title:  address,
          content: + pt.longitude  + ", " + pt.latitude ,
          location: pt
        });
        }
      if (_this.clickon){
        console.log(_this.clickon)
        _this.mapExtentChange=_this.mainmap.on("click", function(evt){
        _this.search.clear();
        _this.mainmap.popup.clear();
        if(_this.search.activeSource){
          var geocoder = _this.search.activeSource.locator;
          var params = {
            location: evt.mapPoint
          };
          geocoder.locationToAddress(params).then(function(response){
      ...
      }else if (!_this.clickon){
        console.log(_this.clickon)
          _this.mapExtentChange.remove()
      }
    },
    distanbutton:function(event){
      var _this=this
        _this.setActiveWidget(null);
        if (!event.currentTarget.classList.contains("active")) {
                _this.setActiveWidget("distance");

            } else {
              console.log('hi')
                _this.setActiveButton(null);
            }

      },
    areabutton:function(event){
    ...
    setActiveWidget:function(type) {
      var _this=this
        switch (type) {
          case "distance":
            _this.activeWidget = new _this.DistanceMeasurement2D({
            view: _this.mainmap
                  });
            ...
            _this.mainmap.ui.add(_this.activeWidget, "top-right");
            this.setActiveButton(document.getElementById("distanceButton"));
          break;
          case "area":
            _this.activeWidget = new _this.AreaMeasurement2D({
              view:  _this.mainmap
            });
          ...
          break;
              ...
              if (_this.activeWidget) {
                console.log('test')
                _this.mainmap.ui.remove(_this.activeWidget);
                _this.activeWidget.destroy();
                _this.activeWidget = null;
              }
          break;
        }
      },
    ...
    drawtool:function(){
      ...
      var view=_this.mainmap
        ...
        function createCircle(event) {
            ...
            var dis=center.distance(new _this.Point({
                hasZ: false,
                hasM: false,
                x:vertices[1][0],
                y:vertices[1][1],
    ...
    doquery:function(graphics){
      var _this=this
      var geometry = graphics.geometry;
      var queryTask = new _this.QueryTask('http://localhost:6080/arcgis/rest/services/monster/monster/FeatureServer/0');
      var query = new _this.Query(); // 创建查询参数对象
          query.geometry = geometry; // 空间查询的几何对象
          query.outFields = ["*"]; // 服务器返回所有信息
          query.outSpatialReference = _this.maplayer.spatialReference; // 空间参考信息
          query.spatialRelationship = _this.Query.SPATIAL_REL_INTERSECTS; // 设置查询的标准
          query.returnGeometry = true;
          queryTask.execute(query)
              .then(result => {
                  _this.mainmap.graphics.removeAll();
                  result.features.forEach(item => {
                      _this.mainmap.graphics.add(_this.createFeatures(item));
                  })
              })
  ...
  beforeMount: function () {
		...
		district: function(){
			...
			this.addre=this.prov+this.city+this.district
			this.locationaddre()
    	},
		clickon:function(){
		this.clickaddress()
  ...
  mounted() {
      const options = {
          url: 'http://localhost/arcgis_js_api/library/4.15/dojo/dojo.js'
         //url:' https://js.arcgis.com/4.15/dojo/dojo.js'
      }
      esriLoader.loadModules([
        ...
        GraphicsLayer
          ...
          this.DistanceMeasurement2D=DistanceMeasurement2D,
          this.AreaMeasurement2D=AreaMeasurement2D
...
<style>
@import url('http://localhost/arcgis_js_api/library/4.15/esri/css/main.css');
/* @import url('https://js.arcgis.com/4.15/esri/css/main.css'); */
#viewDiv{
    position: absolute;
    width: calc(100% - 50px);
...
.esri-search__clear-button{
    ...
    border-top: 2px solid rgb(0, 150, 250) !important;
    border-right: 2px solid rgb(0, 150, 250) !important;
    border-bottom: 2px solid rgb(0, 150, 250) !important;
}
